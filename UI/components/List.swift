...
open class List: UIComponent, ListItemDelegate {
    ...
    public func validate(provider: Array<Any>, position: @escaping (ListItemRenderer) ->Bool) {
        ...
        DispatchQueue.main.async {
            ...
            if let renderer = self.udRenderer {
                componentCLS = NSClassFromString("\(renderer.renderer!)") as? ListItemRenderer.Type
    ...
    private func reset() {
        if (components.count > 0) {
            print(" ‼️  List::reset() - NEED REALIZATION")
        }
    }
    ...
    public func selectItem(item: ListItemRenderer) {
